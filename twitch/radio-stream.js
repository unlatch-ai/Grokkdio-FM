...
async function startRadioStream() {
  ...
  const ffmpegArgs = [
    ...
    '-f', 's16le',
    '-ar', TTS_SAMPLE_RATE.toString(),
    '-ac', TTS_CHANNELS.toString(),
    '-i', 'pipe:0',
    ...
    // Add text overlay
    '-vf', "drawtext=text='XAI Radio Test Stream':fontsize=48:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2",
    ...
    '-c:a', 'aac',
    '-b:a', '128k',
    '-ar', '48000', // resample to 48kHz for Twitch
      ...
      try {
        ...
        if (!ffmpeg.stdin.destroyed) {
          ffmpeg.stdin.write(audioBuffer, (err) => {
            if (err) {
              console.error('Error writing audio to ffmpeg stdin:', err.message);
            }
          });
        }
